...
private:
    ...
    std::vector<std::pair<double, double>> results; // (czas, błąd względny)
...
public:
    ...
    size_t lastSlash = xmlFile.find_last_of("/\\");
    if (lastSlash != std::string::npos) {
        filename = xmlFile.substr(lastSlash + 1);
    }

    // Wczytaj graf i stwórz macierz
    try {
        Graph graph = Parser::loadGraphFromXML(xmlFile);  // używamy pełnej ścieżki
        matrix.fromGraph(graph);
        numCities = matrix.getSize();

        // Ustaw parametry na podstawie nazwy pliku
        if(filename == "ftv47.xml") {
            optimal_distance = 1776;
            timeLimit = 120;
        } else if(filename == "ftv170.xml") {
            optimal_distance = 2755;
            timeLimit = 240;
        } else if (filename == "rbg403.xml") {
            optimal_distance = 2465;
            timeLimit = 360;
        } else {
            // Wyświetl więcej informacji diagnostycznych
            std::cerr << "Próba otwarcia pliku: " << xmlFile << std::endl;
            std::cerr << "Wyodrębniona nazwa pliku: " << filename << std::endl;
        ...
        std::cerr << "Pełna ścieżka: " << xmlFile << std::endl;
        std::cerr << "Nazwa pliku: " << filename << std::endl;
        throw;
    }
}
    // obliczanie fitness
    double calculateFitness(std::vector<int>& chromosome) const;
    ...
    void mutationInsert(std::vector<int>& chromosome);
    ...
    std::vector<std::pair<double, double>> getEvolutionData() const;
